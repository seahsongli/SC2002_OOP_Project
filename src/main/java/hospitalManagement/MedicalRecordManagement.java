...
public class MedicalRecordManagement {
    ...
    public void updateContactNumber(String patientId, String patientName, String phoneNumber) {
        for (int i = 0; i < medicalRecords.size(); i++) {
            if (medicalRecords.get(i).getPatientId().equals(patientId)) {
                ...
                System.out.println("Contact number updated for patient " + patientName + " with ID " + patientId
                        + " to " + phoneNumber);
            }
        }
    }
    ...
    public void updateEmail(String patientId, String patientName, String email) {
        for (int i = 0; i < medicalRecords.size(); i++) {
            if (medicalRecords.get(i).getPatientId().equals(patientId)) {
                ...
                System.out
                        .println("Email updated for patient " + patientName + " with ID " + patientId + " to " + email);
            }
        }
    }
    ...
    // might not need this since we can just use the viewPersonalMedicalRecord
    // method where doctors can view their own patient medical records.
    ...
    protected void setPatientTreatmentPlan(String patientId, String patientName, String treatmentPlan) {
        for (int i = 0; i < medicalRecords.size(); i++) {
            if (medicalRecords.get(i).getPatientId() == patientId) {
                medicalRecords.get(i).addCurrentTreatment(treatmentPlan);
                System.out.println(
                        "Medical record treatment plan updated for patient " + patientName + " with ID " + patientId);
            }
        }
    }
    ...
    protected void setPatientMedicalPrescription(String patientId, String patientName, String prescription) {
        for (int i = 0; i < medicalRecords.size(); i++) {
            if (medicalRecords.get(i).getPatientId().equals(patientId)) {
                medicalRecords.get(i).addPrescription(prescription);
                System.out.println(
                        "Medical record prescription updated for patient " + patientName + " with ID " + patientId);
            }
        }
    }

    // Below 3 methods are used to append new diagnosis and prescription and
    // treatment plans to the existing ones.
    protected void addNewMedicalDiagnosis(String patientId, String patientName, String diagnosis) {
        for (int i = 0; i < medicalRecords.size(); i++) {
            if (medicalRecords.get(i).getPatientId().equals(patientId)
                    && medicalRecords.get(i).getPatientName().equals(patientName)) {
                medicalRecords.get(i).addCurrentDiagnosis(diagnosis);
                System.out.println(
                        "New medical record diagnosis added for patient " + patientName + " with ID " + patientId);
            }
        }
    }
    ...
    protected void addNewMedicalPrescription(String patientId, String patientName, String prescription) {
        for (int i = 0; i < medicalRecords.size(); i++) {
            if (medicalRecords.get(i).getPatientId().equals(patientId)
                ...
                medicalRecords.get(i).addPrescription(prescription);
                System.out.println(
                        "New medical record prescription added for patient " + patientName + " with ID " + patientId);
            }
        }
    }
    ...
    protected void addNewTreatmentPlan(String patientId, String patientName, String treatmentPlan) {
        for (int i = 0; i < medicalRecords.size(); i++) {
            if (medicalRecords.get(i).getPatientId().equals(patientId)
                ...
                medicalRecords.get(i).addCurrentTreatment(treatmentPlan);
                System.out.println(
                        "New medical record treatment plan added for patient " + patientName + " with ID " + patientId);
            }
        }
    }
                ...
                switch (choice) {
                    case 1:
                        ...
                        if (medicalRecords.get(i).getPastDiagnoses().contains(pastDiagnosis)) {
                            System.out.println("Past diagnosis already exists for patient " + patientName + " with ID "
                                    + patientId);
                            return;
                        }
                    ...
                    case 2:
                        ...
                        if (medicalRecords.get(i).getPastTreatments().contains(pastTreatmentPlan)) {
                            System.out.println("Past treatment plan already exists for patient " + patientName
                                    + " with ID " + patientId);
                            return;
                        }
                        medicalRecords.get(i).addPastTreatments(pastTreatmentPlan);
                        System.out.println(
                                "Past treatment plan added for patient " + patientName + " with ID " + patientId);
                        break;
                    case 3:
                        ...
                        if (!medicalRecords.get(i).getPastDiagnoses().contains(pastDiagnosis)) {
                            System.out.println(
                                    "Past diagnosis not found for patient " + patientName + " with ID " + patientId);
                            return;
                        }
                        medicalRecords.get(i).removePastDiagnosis(pastDiagnosis);
                        System.out
                                .println("Past diagnosis removed for patient " + patientName + " with ID " + patientId);
                        break;
                    case 4:
                        System.out.println("Enter past treatment plan to remove: ");
                        pastTreatmentPlan = sc.nextLine().trim();
                        if (!medicalRecords.get(i).getPastTreatments().contains(pastTreatmentPlan)) {
                            System.out.println("Past treatment plan not found for patient " + patientName + " with ID "
                                    + patientId);
                            return;
                        }
                        medicalRecords.get(i).removePastTreatment(pastTreatmentPlan);
                        System.out.println(
                                "Past treatment plan removed for patient " + patientName + " with ID " + patientId);
                        break;
                    default:
